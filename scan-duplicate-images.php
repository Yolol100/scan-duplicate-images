 ...
 * Description: Finds images that are used across multiple posts/pages, including content, featured images, Gutenberg block attributes, srcset/background references, and ACF fields.
 * Version: 3.1.0
...
if ( defined( 'DIUS_PLUGIN_FILE' ) || function_exists( 'dius_get_scannable_post_types' ) ) {
	if ( is_admin() && ! function_exists( 'dius_bootstrap_conflict_notice' ) ) {
		...
		function dius_bootstrap_conflict_notice() {
			echo '<div class="notice notice-error"><p>' . esc_html__( 'Duplicate Image Usage Scanner was not loaded because another copy or conflicting version is already active.', 'scan-duplicate-images' ) . '</p></div>';
		}
